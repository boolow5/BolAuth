...
import (
	...
	"github.com/boolow5/BolAuth/models"
...
/*role := models.Role{}
role.Title = "Test this"
role.Done = false
role.TimeAdded = time.Now()
fmt.Println(role.ID)
added, err := role.Add()
fmt.Println("added = ", added, "err = ", err)
if added == true || err != nil {
  fmt.Println("added role")
} else {
  fmt.Println("failed to add role", err)
}
t, err := models.GetRoleByID(1)
fmt.Println(t.String())
fmt.Println(err)
//t.Title = "Nothing"
//t.Update()
t.Delete()
ts, err := models.GetAllRoles()
fmt.Println("Roles:")
fmt.Println(ts)
fmt.Println(err)
fmt.Println("Welcome to BolRole!")*/
